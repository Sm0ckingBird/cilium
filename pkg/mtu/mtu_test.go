...
func (m *MTUSuite) TestNewConfiguration(c *C) {
	...
	conf := NewConfiguration(0, false, false, false, 0, nil, false)
	...
	conf = NewConfiguration(0, false, false, false, 1400, nil, false)
	c.Assert(conf.GetDeviceMTU(), Equals, 1400)
	c.Assert(conf.GetRouteMTU(), Equals, conf.GetDeviceMTU())

	// Add routes with tunnel
	conf = NewConfiguration(0, false, true, false, 1400, nil, false)
	c.Assert(conf.GetDeviceMTU(), Equals, 1400)
	...
	conf = NewConfiguration(16, true, false, false, 1400, nil, false)
	c.Assert(conf.GetDeviceMTU(), Equals, 1400)
	c.Assert(conf.GetRouteMTU(), Equals, conf.GetDeviceMTU()-EncryptionIPsecOverhead)

	conf = NewConfiguration(32, true, false, false, 1400, nil, false)
	c.Assert(conf.GetDeviceMTU(), Equals, 1400)
	c.Assert(conf.GetRouteMTU(), Equals, conf.GetDeviceMTU()-(EncryptionIPsecOverhead+16))

	conf = NewConfiguration(12, true, false, false, 1400, nil, false)
	c.Assert(conf.GetDeviceMTU(), Equals, 1400)
	...
	conf = NewConfiguration(16, true, true, false, 1400, nil, false)
	c.Assert(conf.GetDeviceMTU(), Equals, 1400)
	c.Assert(conf.GetRouteMTU(), Equals, conf.GetDeviceMTU()-(TunnelOverhead+EncryptionIPsecOverhead))

	conf = NewConfiguration(32, true, true, false, 1400, nil, false)
	c.Assert(conf.GetDeviceMTU(), Equals, 1400)
	...
	conf = NewConfiguration(32, false, false, true, 1400, nil, false)
	c.Assert(conf.GetDeviceMTU(), Equals, 1400)
	c.Assert(conf.GetRouteMTU(), Equals, conf.GetDeviceMTU()-WireguardOverhead)

	conf = NewConfiguration(32, false, true, true, 1400, nil, false)
	c.Assert(conf.GetDeviceMTU(), Equals, 1400)
	c.Assert(conf.GetRouteMTU(), Equals, conf.GetDeviceMTU()-WireguardOverhead)
	...
	conf = NewConfiguration(0, true, true, false, 1400, testIP1, false)
	c.Assert(conf.GetDeviceMTU(), Equals, 1400)

	conf = NewConfiguration(0, true, true, false, 1400, testIP1, true)
	c.Assert(conf.GetDeviceMTU(), Equals, 1372)

	conf = NewConfiguration(0, true, true, false, 0, testIP1, false)
	c.Assert(conf.GetDeviceMTU(), Equals, 1500)

	// Assuming loopback interface always exists and has mtu=65536
	conf = NewConfiguration(0, true, true, false, 0, testIP2, false)
